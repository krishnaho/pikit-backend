...
class PromotionController extends Controller
{
    public function viewAllBanners()
    {
        ...
        return view('admin.banners.viewBanner', [
            'banners' => $banners,
    ...
    {
        $banner = Banner::with('restaurant_category')->find($id);
        $restaurantCategories = RestaurantCategory::where('is_active', 1)->get();
        $bannerItems = $banner->items()->pluck('item_id')->toArray();
        $bannerRestaurants = $banner->restaurants()->pluck('restaurant_id')->toArray();
        $restaurants = Restaurant::where('city_id', $banner->city_id)->where('restaurant_category_id', $banner->restaurant_category_id)->where('is_deleted', 0)->get();
        $cities = Auth::user()->cities;
        $restaurantIds = Restaurant::where('city_id', $banner->city_id)->where('restaurant_category_id', $banner->restaurant_category_id)->where('is_deleted', 0)->pluck('id')->toArray();
        ...
        try {
            if ($banner) {
                return view('admin.banners.editBanner', [
                    ...
                    'items' => $items,
                    'restaurants' => $restaurants,
                    'bannerRestaurants' => $bannerRestaurants,
                ]);
            } else {
    ...
    public function getRestaurantCategoryItems(Request $request)
    {
        if ($request->city_id) {
            $restaurantIds = Restaurant::where('city_id', $request->city_id)
                ->where('restaurant_category_id', $request->restaurant_category_id)->where('is_deleted', 0)
                ->pluck('id')->toArray();
        } else {
            $restaurantIds = Restaurant::where('restaurant_category_id', $request->restaurant_category_id)
    ...
    public function getRestaurantCategoryRestaurant(Request $request)
    {
        if ($request->city_id) {
            $restaurants = Restaurant::where('is_deleted', 0)
                ->where('restaurant_category_id', $request->restaurant_category_id)->where('city_id', $request->city_id)->get();
        } else {
            $restaurants = Restaurant::where('is_deleted', 0)
                ->where('restaurant_category_id', $request->restaurant_category_id)->get();
        }
    ...
    public function viewAllCoupons()
    {
        $cities = Auth::user()->cities;
        $cityIds = $cities->pluck('id')->toArray();
        $coupons = Coupon::whereIn('city_id', $cityIds)->get();
        $restaurantCategories = RestaurantCategory::where('is_active', 1)->get();
        $restaurants = Restaurant::where('is_active', 1)->get();
        $items = Item::where('is_active', 1)->get();
        return view('admin.coupons.viewCoupon', [
            'coupons' => $coupons,
            'restaurantCategories' => $restaurantCategories,
            'cities' => $cities,
            'restaurants' => $restaurants,
    ...
    public function editCoupon($id)
    {
        ...
        $restaurants = Restaurant::where('city_id', $coupon->city_id)->where('is_deleted', 0)->get();
        $cities = Auth::user()->cities;
